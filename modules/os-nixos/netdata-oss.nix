# COPIED from nixos-24.11 release to be compatible with pre-v2 netdata (necessary to have the local web UI)
# The service is renamed `netdata-oss` (from `netdata`) to avoid conflict with the existing service for v2.
#
# /!\ Will need to keep the module updated somehow with changes to the used NixOS dependencies. /!\
...
let
  cfg = config.services.netdata-oss;
...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    services.netdata-oss.python.extraPackages = lib.mkIf cfg.python.recommendedPythonPackages (ps: [
      ps.requests
      ps.pandas
      ps.numpy
      ps.psycopg2
    ...
    services.netdata-oss.configDir.".opt-out-from-anonymous-statistics" = mkIf (
      !cfg.enableAnalyticsReporting
    ) (pkgs.writeText ".opt-out-from-anonymous-statistics" "");
    environment.etc."netdata/netdata.conf".source = configFile;
    environment.etc."netdata/conf.d".source = configDirectory;

    systemd.services.netdata-oss = {
